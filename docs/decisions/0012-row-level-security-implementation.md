...
- `APP_DB_PASSWORD` is a new secret that must be provisioned in every environment (local `.env`, `dev`, production) and rotated independently of `POSTGRES_PASSWORD`.
