...
runsRouter.use('*', orgAuth({ admin: true }))
...
runsRouter.get('/', async (c) => {
  ...
  // to organizationId via withTenant's set_config.
...
runsRouter.get('/:id', async (c) => {
  ...
  // lookups to organizationId — a run from another tenant simply
...
runsRouter.patch('/:id/steps/:stepId', zValidator('json', reassignRunStepSchema), async (c) => {
  const { organizationId } = c.get('orgAuth')
  ...
  if (assigneeId && !(await assertValidAssignee(assigneeId, organizationId))) {
